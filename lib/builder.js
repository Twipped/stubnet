...
function Step(type, options) {
	...
	this.pass = function () {
		(typeof options.pass === 'function' && options.pass || noop).call(this, this.name);
		this.done.call(this);
	};
...
Builder.prototype = extend(Builder.prototype, {
	...
	expectData: function expectData (data, message) {
		return this.expectDataFrom(0, data, message);
	},
	...
	expectNoData: function expectNoData (message) {
		return this.expectNoDataFrom(0, message);
	},
	...
	expectDisconnect: function expectDisconnect (message) {
		return this.expectDisconnectBy(0, message);
	},
	...
	thenSend: function thenSend (data, message) {
		return this.thenSendTo(0, data, message);
	},

	thenClose: function thenClose (index, options) {
		debug('pushing', 'close', index, options);
		...
		var step = new Step('close', options);
		step.socketIndex = index;
