...
module.exports = function runner (baseSteps) {
	...
	function failed(assertion) {
		...
		shutdown(function () {
			debug('returning to failed from shutdown', steps.length);
	...
	function process () {
		...
		debug('process', currentStep && currentStep.type);
	...
	var stepChecks = {
		...
		'send': function (currentStep) {
			...
			// we're waiting for the buffer to flush, don't do anything.
			if (currentStep.writing) return;
	...
	return {
		abort: function () {
			return failed(new AssertionError({
				message:  'Test was aborted externally'
			}));
		}
	};
};
